        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg mt-3">
                <div class="p-6 text-gray-900 flex justify-between content-center">
                    ...
                    <form method="POST" action="{{route('matches.update', $match)}}" class="w-full">
                        @csrf
                        @method('PUT')
                        <div class="mb-6">
                            ...
                            <select id="home_team" name="home_team"
                                ...
                                <option value="{{$team->id}}" @if (old('home_team', $match->home_team) == $team->id)
                                    selected
                                    @endif>{{$team->name}}</option>
                                @endforeach
                            </select>
                            @error('home_team')
                            <div class="p-3 mb-3 text-sm text-red-800" role="alert">
                                {{$message}}
                            </div>
                            @enderror
                        </div>
                        <div class="mb-6">
                            ...
                            <select id="away_team" name="away_team"
                                ...
                                @foreach ($teams as $team)
                                <option value="{{$team->id}}" @if (old('away_team', $match->away_team) == $team->id)
                                    selected
                                    @endif>{{$team->name}}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-6">
                            ...
                            <input type="date" id="game_date" name="game_date"
                                value="{{old('game_date', $match->game_date)}}"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
                            @error('game_date')
                            <div class="p-3 mb-3 text-sm text-red-800" role="alert">
                                {{$message}}
